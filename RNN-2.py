...
class MeteoroRNN:
    ...
    def calculateLoss(self, inputData, OutputData):
        ...
        while i < inputData.shape[0] - T:
            ...
            output = 0.0
            for t in range(T):
                hiddenState = self.HiddenLayerActivation(
                    np.dot(self.U, inputData[i + t].reshape(-1, 1)) + np.dot(self.W, hidden_state_t_minus_1.reshape(-1,
                                                                                                                    1)) + self.bh)
        ...
        for epoch in range(self.epoch):
            ...
            while i < trainData.shape[0] - T:
                ...
                for t in range(T):
                    ...
                    aggInput[t] = (np.dot(U, trainData[i + t].reshape(-1, 1)) + np.dot(W,
                                                                                       hidden_state_t_minus_1.reshape(
                                                                                           -1,
                                                                                           1)) + bh).flatten()  # U-hidden_dim x input_size ,Xtrain[i]-input_size x 1 W-hidden_dim x hidden_dim ,hidden_state-hidden_dim x 1
                    hiddenState[t] = self.HiddenLayerActivation(aggInput[t])  # hidden_dim x 1
                    aggOutput[t] = np.dot(V, hiddenState[t].reshape(-1,
                                                                    1)) + by  # V-1 x hidden_dim hidden hiddenState[t]- hiddem_dim x 1
                ...
                for t in reversed(range(T)):
                    ...
                    if (t < T - 1):
                        dh = np.dot(W * d_input_agg[t + 1].reshape(-1, 1), dh_next) + (
                                    V.reshape(-1, 1) * d_output_agg[t] * error[
                                t])  # dh-hidden_dim x 1 , W-hiddem_dim x hidden_dim , hidden
                    else:
                        dh = V.reshape(-1, 1) * d_output_agg[t] * error[t]
                    # dj/dw=dJ/dh dot (h[t-1]*d_input_agg)

                    deltaW += np.transpose(
                        np.dot(dh * d_input_agg[t].reshape(-1, 1), hiddenState[t - 1].reshape(1, -1)))
                    # dj/du=dj/dh dot (x[t]*d_input_agg )
                    deltaU += np.array(np.dot(dh * d_input_agg[t], trainData[i + t].reshape(1, -1)),
                                       dtype=np.float64)  #
...
class DataPreprocessor:
    ...
    def transform(self, data):
        ...
        processed_data = pd.concat([month_encoded, pd.DataFrame(normalized_temp, columns=['Norm_AvgTemperature'])],
                                   axis=1)
...
url_train = 'https://raw.githubusercontent.com/arkitepulse/MeteoroRNN/main/train.csv'
url_test = 'https://raw.githubusercontent.com/arkitepulse/MeteoroRNN/main/test.csv'

trainData = ModifyData(url_train)
validationData = ModifyData(url_test)

meteoroRNN = MeteoroRNN(learningRate, epoch, sequence_length, hiddenDim, minClip, maxClip, trainData.shape[1])
meteoroRNN.train(trainData.values, trainData['Norm_AvgTemperature'].values, validationData.values,
                 validationData['Norm_AvgTemperature'].values)
